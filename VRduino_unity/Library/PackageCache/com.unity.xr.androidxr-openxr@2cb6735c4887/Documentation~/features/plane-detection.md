...
Cast the `void* planePtr` to an `XrTrackableANDROID` handle in C++ using the following example code:

```cpp
// Marshal the native plane data from the XRPlane.nativePtr in C#
UnityXRNativePlane nativePlane;
XrTrackableANDROID androidObjectHandle = static_cast<XrTrackableANDROID>(nativePlane.planePtr);
```
