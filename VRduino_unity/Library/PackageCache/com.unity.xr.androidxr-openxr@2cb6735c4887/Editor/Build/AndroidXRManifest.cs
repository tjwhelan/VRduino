...
{
    internal class AndroidXRManifest : IPostGenerateGradleAndroidProject
    {
        ...
        internal static readonly string k_EyeTrackingHardwareFeature = "android.hardware.xr.input.eye_tracking";
        ...
        internal static readonly XName k_AndroidName = k_Android + "name";
        static readonly XName k_AndroidValue = k_Android + "value";
        internal static readonly XName k_AndroidRequired = k_Android + "required";
        ...
        public void OnPostGenerateGradleAndroidProject(string gradleProjectPath)
        {
            ...
            if (foveatedRenderingFeature != null && foveatedRenderingFeature.enabled)
            {
                ...
                // ETFR gracefully degrades to FFR, and the Play Store filters out items that appear
                // to require eye tracking on devices that don't have it. If there is no
                // `required=false` entry in the manifest, then the Play Store assumes it is
                // required from the permission entry.
                // https://developer.android.com/guide/topics/manifest/uses-permission-element
                AddFeature(manifest, k_EyeTrackingHardwareFeature, required: false);
            }
        ...
        static void AddElement(XElement container, string elementName, string value, bool required = true, string version = null)
        {
            var existingElement = container
                .Elements()
                .Where(x => x.Name == elementName && x.Attribute(k_AndroidName).Value == value)
                .DefaultIfEmpty(null)
                ?.First();

            if (existingElement != null)
            {
                // necessary to override the `required` attribute if the feature is required. By
                // default, `required` is true, so we remove it. Optional (`required=false`)
                // features will just return without modification because the element is already
                // defined.
                if (required)
                    existingElement.SetAttributeValue(k_AndroidRequired, null);
        ...
        static void AddPermission(XElement manifest, string permission)
        {
            AddElement(manifest, k_UsesPermission, permission);
        }

        internal static void AddFeature(XElement manifest, string feature, bool required = true, string version = null)
        {
