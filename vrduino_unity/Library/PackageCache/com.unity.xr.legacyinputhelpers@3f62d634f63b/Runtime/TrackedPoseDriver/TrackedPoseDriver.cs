...
namespace UnityEngine.SpatialTracking
{
    internal class TrackedPoseDriverDataDescription
    {
        ...
        internal static readonly List<PoseData> DeviceData = new List<PoseData>
        {
            // Generic XR Device
            new PoseData
            {
    ...
    {
#if ENABLE_AR || ENABLE_VR
        static readonly List<XR.XRNodeState> s_NodeStates = new List<XR.XRNodeState>();

        /// <summary>The GetNodePoseData method is used to query data from the XRNode subsystem based on the provided node.</summary>
        /// <param name = "node" > The node to request data for.</param>
        /// <param name = "resultPose" > The resulting pose data.</param>
        /// <returns>Returns a bitflag which represents which data has been retrieved from the provided node</returns>
        internal static PoseDataFlags GetNodePoseData(XR.XRNode node, out Pose resultPose)
        {
            PoseDataFlags retData = PoseDataFlags.NoData;
            XR.InputTracking.GetNodeStates(s_NodeStates);
            foreach (XR.XRNodeState nodeState in s_NodeStates)
            {
